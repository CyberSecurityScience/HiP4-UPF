...
pub enum PDRUpdate {
    ...
    SetMaidVolThreshold(requests_generated::upfdriver::requests::SetMaidVolThresholdArgs),
}

impl PDRUpdate {
    pub fn set_vol_thres(&mut self, ma_id: u32, vol_thres: u64) {
        match self {
            PDRUpdate::DL_N6_SimpleInsert(r) => {
                if r.data_ma_id == ma_id {
                    r.vol_thres = vol_thres;
                }
            },
            ...
            PDRUpdate::SetMaidVolThreshold(r) => {
                if r.ma_id == ma_id {
                    r.bytes = vol_thres;
                }
            },
...
pub async fn send_pdr_update(updates: Vec<PDRUpdate>, delay_budget_us: u64) -> Result<(), i32> {
    ...
    for update in updates {
        let request_args = match update {
            ...
            PDRUpdate::SetMaidVolThreshold(_) => unreachable!(),
        };
