        ...
        for item in to_mod.into_iter() {
            ...
            if (item.update_qer.is_some() || item.update_action.is_some()) && item.update_matches.is_none() {
                ...
                for m in item.old_matches {
                    if let Some(update) = m.generate_upf_driver_update_update(maid, item.maid, global_qer_id) {
                        upf_driver_table_updates.push(update);
                    }
                    to_del_maids.push(maid);
                }
            }
            if let Some(match_update) = item.update_matches {
                ...
                for m in match_update.new_matches {
                    if let Some(update) = m.generate_upf_driver_update_insert(maid, global_qer_id) {
                        upf_driver_table_updates.push(update);
                    }
                }
            }
