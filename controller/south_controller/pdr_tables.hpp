...
struct CopyablePDROperation {
    ...
    union {
        ...
        InsertOrUpdatePerMaidURR urrOp;
    };
};
    ...
    CopyableTransaction(UPFDriver::Requests::Transaction const *const src) {
        ...
        for (usize i(0) ; i != size; ++i) {
            ...
            switch (t) {
                ...
                break;
                case UPFDriver::Requests::RequestUnion_InsertOrUpdatePerMaidURR:
                {
                    op.type = RequestTransactionType::InsertOrUpdatePerMaidURRType;
                    op.urrOp.ma_id = item->request_as_InsertOrUpdatePerMaidURR()->ma_id();
                    op.urrOp.vol_thres = item->request_as_InsertOrUpdatePerMaidURR()->vol_thres();
                    op.urrOp.vol_thres_id = item->request_as_InsertOrUpdatePerMaidURR()->vol_thres_id();
                    op.urrOp.time_thres = item->request_as_InsertOrUpdatePerMaidURR()->time_thres();
                    op.urrOp.period_val = item->request_as_InsertOrUpdatePerMaidURR()->period_val();
                    op.urrOp.flags = item->request_as_InsertOrUpdatePerMaidURR()->flags();
                }
                break;
        ...
        for (auto const& op : ops) {
            switch (op.type) {
                case RequestTransactionType::UL_N6_SimpleInsertType:
                {
                    ...
                    new_ma_ids.push_back(op.UL_N6_SimpleInsertOp.data_ma_id);
                ...
                case RequestTransactionType::UL_N6_SimpleUpdateType:
                {
                    ...
                    my_assert(data->setValue(id_data_qer_id, uint64_t(op.UL_N6_SimpleUpdateOp.data_qer_id)));
                    if (op.UL_N6_SimpleUpdateOp.data_ma_id != op.UL_N6_SimpleUpdateOp.data_old_ma_id) {
                        // if we only update QER_ID we don't update MAID
                        new_ma_ids.push_back(op.UL_N6_SimpleUpdateOp.data_ma_id);
        ...
        for (auto const& op : ops) {
            switch (op.type) {
                case RequestTransactionType::DL_N6_SimpleInsertType:
                {
                    ...
                    new_ma_ids.push_back(op.DL_N6_SimpleInsertOp.data_ma_id);
                    return_if_failed(table->tableEntryAdd(*session, dev_tgt, flags, *key, *data));
                }
                break;
                case RequestTransactionType::DL_N6_SimpleUpdateType:
                {
                    ...
                    if (op.DL_N6_SimpleUpdateOp.data_ma_id != op.DL_N6_SimpleUpdateOp.data_old_ma_id) {
                        // if we only update QER_ID we don't update MAID
                        new_ma_ids.push_back(op.DL_N6_SimpleUpdateOp.data_ma_id);
...
struct PDRTables {
    ...
    bf_status_t batch_update(
        ...
        bf_rt_target_t dev_tgt,
        std::vector<CopyablePDROperation> const& ops,
        std::vector<std::uint32_t>& out_new_ma_ids,
        std::vector<std::uint32_t>& out_old_ma_ids,
        std::vector<InsertOrUpdatePerMaidURR>& out_urr_updates
    ) {
        // update PDR tables
        return_if_failed(ul_n6_simple.batch_update(session, dev_tgt, ops, out_new_ma_ids, out_old_ma_ids));
        return_if_failed(dl_n6_simple.batch_update(session, dev_tgt, ops, out_new_ma_ids, out_old_ma_ids));
        // extract all URR updates
        for (auto const& op : ops) {
            if (op.type == RequestTransactionType::InsertOrUpdatePerMaidURRType) {
                out_urr_updates.push_back(op.urrOp);
            }
        }
        return BF_SUCCESS;
    }
