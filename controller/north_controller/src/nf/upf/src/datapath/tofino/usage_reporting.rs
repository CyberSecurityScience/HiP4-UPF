...
use libpfcp::{models::{DurationMeasurement, TimeOfFirstPacket, TimeOfLastPacket, VolumeMeasurement, URR_ID, UR_SEQN}, IDAllocator};
...
use super::{response_generated::upfdriver::response::CounterValueArgs, upf_driver_interface::PDRUpdate, TofinoBackendError};
...
pub async fn add_or_update_usage_reporting(seid: u64, urr: FlattenedURR, vol_thres_alloc: Arc<Mutex<IDAllocator<u32>>>) {
	if let Some(context) = USAGE_REPORTING_GLOBAL_CONTEXT_PFCP_MAP.get(&seid) {
		...
		if let Some(existing_urr) = context_guard.urr_entries.get_mut(&urr.urr_id) {
			existing_urr.apply_urr(&urr, ts, vol_thres_alloc).await;
		} else {
			let mut new_urr = SingleUsageReportingEntry::new(seid, urr.urr_id);
			new_urr.apply_urr(&urr, ts, vol_thres_alloc).await;
			context_guard.urr_entries.insert(urr.urr_id, new_urr);
		}
	}
}

pub async fn delete_usage_reporting(seid: u64, urr_id: u32, vol_thres_alloc: Arc<Mutex<IDAllocator<u32>>>) -> Result<Option<BackendReport>, TofinoBackendError> {
	let mut ret = None;
	if let Some(context) = USAGE_REPORTING_GLOBAL_CONTEXT_PFCP_MAP.get(&seid) {
		let mut context_guard = context.write().await;
		if let Some(mut urr_entry) = context_guard.urr_entries.remove(&urr_id) {
			...
			if urr_entry.ul_thres_global_id != 0 || urr_entry.dl_thres_global_id != 0 {
				let mut vol_thres_alloc = vol_thres_alloc.lock().await;
				if urr_entry.ul_thres_global_id != 0 {
					// free ID
					vol_thres_alloc.free(urr_entry.ul_thres_global_id);
				}
				if urr_entry.dl_thres_global_id != 0 {
					// free ID
					vol_thres_alloc.free(urr_entry.dl_thres_global_id);
				}
			}
		}
...
pub async fn update_pipeline_urr_association(seid: u64, all_associations: Vec<PdrUsageReportingAssociation>, to_del_maids: Vec<u32>) -> Vec<PDRUpdate> {
	let mut all_maids = vec![];
	let mut south_urrs = vec![];
	...
	if let Some(context) = ctx {
		...
		south_urrs.append(&mut context_guard.get_ma_id_south_controller_urr());
		drop(context_guard);
...
struct SingleUsageReportingEntry {
	...
	pub ul_thres_global_id: u32,
	pub dl_thres_global_id: u32,
...
impl SingleUsageReportingEntry {
	pub fn new(seid: u64, urr_id: u32) -> Self {
		Self {
			...
			ul_thres_global_id: 0,
			dl_thres_global_id: 0,
			preqos_ul_bytes: 0,
			preqos_dl_bytes: 0,
			preqos_ul_pkts: 0,
			preqos_dl_pkts: 0,
	...
	pub async fn apply_urr(&mut self, urr: &FlattenedURR, ts: u64, vol_thres_alloc: Arc<Mutex<IDAllocator<u32>>>) {
		...
		if urr.measurement_method.getVOLUM() != 0 {
			...
			match &urr.volume_threshold {
				Some(vol_thres) if urr.reporting_triggers.getVOLTH() != 0 => {
					...
					let mut vol_thres_alloc = vol_thres_alloc.lock().await;
					if self.ul_thres_global_id == 0 {
						// allocate new one
						self.ul_thres_global_id = vol_thres_alloc.allocate().unwrap();
					}
					if self.dl_thres_global_id == 0 {
						// allocate new one
						self.dl_thres_global_id = vol_thres_alloc.allocate().unwrap();
					}
				}
				_ => {}
			};
...
/// Compute the GCD of a vector of `u64` numbers.
fn gcd_vec(numbers: &Vec<u64>) -> u64 {
...
impl UsageReportingPfcpContext {
	...
	pub fn get_ma_id_south_controller_urr(&self) -> Vec<PDRUpdate> {
		let mut ret = vec![];
		for (ma_id, (is_ul, urr_ids)) in self.maid2reporting_entry.iter() {
			...
			let mut all_time_thres = vec![];
			for urr_id in urr_ids.iter() {
				let urr = self.urr_entries.get(urr_id).unwrap();
				if urr.traffic_usage_thres != u64::MAX {
					all_time_thres.push(urr.traffic_usage_thres);
				}
				if urr.reporting_peroid_us != u64::MAX {
					all_periods.push(urr.reporting_peroid_us);
				}
			}
			let urr_id = urr_ids.iter().next().unwrap();
			let urr = self.urr_entries.get(urr_id).unwrap();

			let mut vol_thres = u64::MAX;
			let mut vol_thres_id = 0;
			
			if *is_ul && urr.ul_threshold != u64::MAX {
				vol_thres = urr.ul_threshold;
				vol_thres_id = urr.ul_thres_global_id;
			}
			if !is_ul && urr.dl_threshold != u64::MAX {
				vol_thres = urr.dl_threshold;
				vol_thres_id = urr.dl_thres_global_id;
			}

			let mut south_urr = super::requests_generated::upfdriver::requests::InsertOrUpdatePerMaidURRArgs {
				ma_id: *ma_id,
				vol_thres: vol_thres,
				vol_thres_id: vol_thres_id,
				time_thres: if all_time_thres.len() != 0 { *all_time_thres.iter().min().unwrap() } else { u64::MAX },
				period_val: if all_periods.len() != 0 { gcd_vec(&all_periods)} else { u64::MAX },
				flags: 0b01,
			};

			ret.push(PDRUpdate::InsertOrUpdatePerMaidURR(south_urr));
		}
		ret
	}
...
pub fn create_usage_reporting_threads(ur_tx: Sender<BackendReport>) {
