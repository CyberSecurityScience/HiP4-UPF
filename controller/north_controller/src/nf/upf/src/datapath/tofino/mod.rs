...
use libpfcp::{
	messages::{CreateFAR, CreatePDR, CreateQER, ForwardingParameters, UpdateFAR, UpdatePDR, UpdateQER},
	models::{ApplyAction, MeasurementMethod, ReportingTriggers, VolumeThreshold, VolumeThresholdFlags, FAR_ID, PDR_ID, QER_ID, URR_ID}, IDAllocator,
};
...
pub struct TofinoBackend {
    ...
    global_vol_thres_id_alloc: Arc<Mutex<IDAllocator<u32>>>,
...
impl BackendInterface for TofinoBackend {
		...
		Self: Sized,
	{
		...
		usage_reporting::create_usage_reporting_threads(settings.ur_tx);
		...
		Box::new(TofinoBackend {
			...
			global_vol_thres_id_alloc: Arc::new(Mutex::new(IDAllocator::new_with_counter(1))),
	...
	async fn update_or_add_forwarding(&self, seid: u64, pipelines: Vec<PacketPipeline>) -> Result<OperationPerfStats, Box<dyn std::error::Error + Send + Sync>> {
		...
		let south_urrs = usage_reporting::update_pipeline_urr_association(seid, pdr_urr_associations, to_del_maids).await;
		for u in south_urrs {
			upf_driver_table_updates.insert(0, u);
		};
	...
	async fn update_or_add_urr(&self, seid: u64, pipelines: Vec<FlattenedURR>) -> Result<(), Box<dyn std::error::Error + Send + Sync>> {
		...
		for p in pipelines.into_iter() {
			usage_reporting::add_or_update_usage_reporting(seid, p, self.global_vol_thres_id_alloc.clone()).await;
		}
	...
	async fn delete_urr(&self, seid: u64, pipelines: Vec<URR_ID>) -> Vec<Option<BackendReport>> {
		...
		for p in pipelines.into_iter() {
			if let Ok(x) = usage_reporting::delete_usage_reporting(seid, p.rule_id, self.global_vol_thres_id_alloc.clone()).await {
				ret.push(x);
			}
		}
